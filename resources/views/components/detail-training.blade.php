        ...
        <div class="lg:col-span-7 space-y-6">
           ...
           ">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span
                        class="inline-flex items-center gap-2 rounded-xl px-3 py-1.5 text-xs font-medium ring-1 {{ $badgeClass }}">
                        {{-- icon status --}}
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 24 24"
                            fill="currentColor">
                            @if ($hasProof)
                ...
                <dl class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div
                        class="rounded-xl border p-4 ring-1 ring-gray-950/5 dark:border-neutral-800 dark:ring-neutral-800">
                        <dt class="text-xs font-medium text-gray-500">PIC</dt>
                        <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $pic }}
                        </dd>
                    </div>
                    <div
                        class="rounded-xl border p-4 ring-1 ring-gray-950/5 dark:border-neutral-800 dark:ring-neutral-800">
                        <dt class="text-xs font-medium text-gray-500 ">Perusahaan</dt>
                        <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $company }}
                        </dd>
                    </div>
                    <div
                        class="rounded-xl border p-4 ring-1 ring-gray-950/5 dark:border-neutral-800 dark:ring-neutral-800">
                        <dt class="text-xs font-medium text-gray-500">Lokasi Pelatihan</dt>
                        <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $prov }},
                            {{ $city }}</dd>
                    </div>
                    <div
                        class="rounded-xl border p-4 ring-1 ring-gray-950/5 dark:border-neutral-800 dark:ring-neutral-800">
                        <dt class="text-xs font-medium text-gray-500">Tanggal Pelatihan</dt>
                        <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $dateSafe }}
                        </dd>
                    </div>
                </dl>
        ...
        <div class="lg:col-span-5 space-y-6">
           ...
           ">
                    <div class="flex items-center justify-between">
                        ...
                        <span
                            class="text-xs text-gray-500 dark:text-neutral-400">{{ $training->approvalFiles?->count() ?? 0 }}
                            file</span>
                    </div>
                        ...
                        <ul class="mt-4 space-y-2">
                            @foreach ($training->approvalFiles as $f)
                                @php
                                    ...
                                    $label = $isProof ? 'Bukti Bayar' : 'Lampiran';
                                    $href = $isProof ? asset('storage/' . $f->proof_payment) : null;
                                @endphp
                                <li
                                    class="group flex items-center justify-between gap-3 rounded-xl border p-3 ring-1 ring-gray-950/5 hover:bg-gray-50 dark:border-neutral-800 dark:ring-neutral-800 dark:hover:bg-neutral-800/40">
                                    <div class="flex items-center gap-3">
                                        {{-- icon file --}}
                                        <div
                                            class="flex h-9 w-9 items-center justify-center rounded-lg bg-gray-100 text-gray-700 ring-1 ring-gray-950/5 dark:bg-neutral-800 dark:text-neutral-200 dark:ring-neutral-700">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4.5 w-4.5"
                                                viewBox="0 0 24 24" fill="currentColor">
                                                <path
                                                    d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zM8 12h8v2H8v-2zm0 4h8v2H8v-2zm6-9h4v.01L14 3.99V7z" />
                                            </svg>
                                        </div>
                                        <div>
                                        ...
                                        @if ($href)
                                            <a href="{{ $href }}" target="_blank"
                                                ...
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5"
                                                    ...
                                                    <path d="M7 17L17 7M7 7h10v10" stroke-width="2"
                                                        stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                            </a>
                                        @else
                                            <span
                                                class="inline-flex items-center rounded-lg bg-gray-100 px-2.5 py-1.5 text-xs font-medium text-gray-500 ring-1 ring-gray-950/5 dark:bg-neutral-800 dark:text-neutral-400 dark:ring-neutral-700">
                                                Tidak tersedia
                                            </span>
                                        @endif
                                    </div>
                    ...
                    @else
                        <div
                            class="mt-4 rounded-xl border border-dashed p-6 text-center text-sm text-gray-500 ring-1 ring-gray-950/5 dark:border-neutral-800 dark:text-neutral-400 dark:ring-neutral-800">
                            Belum ada file.
                        </div>
                    @endif
                </div>
