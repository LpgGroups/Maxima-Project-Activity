        ...
        .done(function (data) {
            ...
            const SwalHTML = `
                ...
                <div class="text-sm bg-gray-50 border border-gray-300 rounded-md p-4 grid grid-cols-[120px,10px,1fr] gap-y-2">
                    <div class="text-right font-bold text-gray-700">Nama PIC</div><div>:</div><div>${data.pic ?? "-"}</div>
                    <div class="text-right font-bold text-gray-700">Perusahaan</div><div>:</div><div>${data.company ?? "-"}</div>
                    <div class="text-right font-bold text-gray-700">Email PIC</div><div>:</div><div>${data.email ?? "-"}</div>
                    <div class="text-right font-bold text-gray-700">No WhatsApp</div><div>:</div><div>${data.phone ?? "-"}</div>
                    <div class="text-right font-bold text-gray-700">Date</div><div>:</div><div>${data.date} - ${data.date_end}</div>
                    <div class="text-right font-bold text-gray-700">Peserta</div><div>:</div><div>${data.participants} Peserta</div>
                </div>
            ...
            Swal.fire({
                title: `<strong>${data.activity}</strong>`,
                html: SwalHTML,
                icon: "info",
                ...
                preConfirm: () => {
                    if (isApproved) {
                        Swal.showValidationMessage('Data sudah disetujui, tidak bisa mengubah status menjadi setuju lagi.');
                        return false;
                    }
                ...
                preDeny: () => {
                    if (!isApproved) {
                        Swal.showValidationMessage('Data belum disetujui, tidak bisa menolak.');
                        return false;
                    }
                    const url = `/dashboard/management/approve/${id}`;
                    return fetch(url, {
